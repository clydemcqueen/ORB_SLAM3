...
KeyFrame::KeyFrame() : mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS)
{
}
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mnMaxY(F.mnMaxY), mK_(F.mK_), mpImuPreintegrated(F.mpImuPreintegrated),
    mImuCalib(F.mImuCalib), mvpMapPoints(F.mvpMapPoints), mpKeyFrameDB(pKFDB),
    mpORBvocabulary(F.mpORBvocabulary), mbFirstConnection(true), mDistCoef(F.mDistCoef), mnDataset(F.mnDataset),
    mHalfBaseline(F.mb/2), mpMap(pMap), mNameFile(F.mNameFile),
    mpCamera(F.mpCamera), mpCamera2(F.mpCamera2),
    mvLeftToRightMatch(F.mvLeftToRightMatch),mvRightToLeftMatch(F.mvRightToLeftMatch), mTlr(F.GetRelativePoseTlr()),
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl())
{
    mnId=nNextId++;
