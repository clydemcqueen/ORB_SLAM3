...
TEST(ORBSLAM3, MonoEvaluationTest) {
  ...
  if (!create_baseline) {
    ...
    while (std::getline(f, line)) {
      ...
      ss >> r.timestamp >> comma >> r.tracking_state >> comma >> r.num_features >> comma >> r.tx >>
          comma >> r.ty >> comma >> r.tz >> comma >> r.qx >> comma >> r.qy >> comma >> r.qz >>
          comma >> r.qw;
      baseline.push_back(r);
    }
    f.close();
    ASSERT_EQ(baseline.size(), image_files.size()) << "Baseline size mismatch with image count.";
  } else {
  ...
  float pos_tolerance = 0.1F;  // Increased to 0.1m due to non-determinism and monocular scale drift
  ...
  for (size_t i = 0; i < image_files.size(); i++) {
    ...
    if (create_baseline) {
      f_out << tframe << "," << state << "," << num_features << "," << trans.x() << "," << trans.y()
            << "," << trans.z() << "," << q.x() << "," << q.y() << "," << q.z() << "," << q.w()
            << "\n";
    } else {
      const FrameResult& b = baseline[i];

      // Allow state 2 (Tracking) to be exactly equal
      // Actually, we should expect the tracking state to be the same, or at least if baseline was
      // tracking, test should track.
